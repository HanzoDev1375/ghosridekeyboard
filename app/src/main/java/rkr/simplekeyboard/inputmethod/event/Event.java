 ...
 *
 * <p>This contains information about the origin of the event, but it is generalized and should
 * represent a software keypress, hardware keypress, or d-pad move alike. Very importantly, this
 * does not necessarily result in inputting one character, or even anything at all - it may be a
 * dead key, it may be a partial input, it may be a special key on the keyboard, it may be a
 * cancellation of a keypress (e.g. in a soft keyboard the finger of the user has slid out of the
 * key), etc. It may also be a batch input from a gesture or handwriting for example. The combiner
...
public class Event {
  ...
  private static final int FLAG_REPEAT = 0x2;
  // This event has already been consumed.
  private static final int FLAG_CONSUMED = 0x4;

  private final int mEventType; // The type of event - one of the constants above
  ...
  public static Event createSoftwareKeypressEvent(
      final int codePoint, final int keyCode, final int x, final int y, final boolean isKeyRepeat) {
    return new Event(
   ...
   * output is a string, possibly with styling. Examples include press on a multi-character key, or
   * combination that outputs a string.
   *
