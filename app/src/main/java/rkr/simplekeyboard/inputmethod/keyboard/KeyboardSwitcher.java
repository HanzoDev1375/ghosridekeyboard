...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
  ...
  public void updateKeyboardTheme(final int uiMode) {
    final boolean themeUpdated =
        updateKeyboardThemeAndContextThemeWrapper(
            mLatinIME, KeyboardTheme.getKeyboardTheme(mLatinIME), uiMode);
    if (themeUpdated && mKeyboardView != null) {
      mLatinIME.setInputView(onCreateInputView(uiMode));
      mKeyboardView.setSpaceTextColor(MaterialColors.getColor(mKeyboardView, com.google.android.material.R.attr.colorOnSurface));
    }
  }

  private boolean updateKeyboardThemeAndContextThemeWrapper(
      final Context context, final KeyboardTheme keyboardTheme, final int uiMode) {
  ...
  private void setKeyboard(final int keyboardId, final KeyboardSwitchState toggleState) {
    ...
    final boolean subtypeChanged =
        (oldKeyboard == null) || !newKeyboard.mId.mSubtype.equals(oldKeyboard.mId.mSubtype);
  ...
  public boolean isImeSuppressedByHardwareKeyboard(
      final SettingsValues settingsValues, final KeyboardSwitchState toggleState) {
    ...
    if (DEBUG_ACTION) {
      Log.d(
          ...
          "requestUpdatingShiftState: "
              + " autoCapsFlags="
              + CapsModeUtils.flagsToString(autoCapsFlags)
              + " recapitalizeMode="
              + RecapitalizeStatus.modeToString(recapitalizeMode));
    }
  ...
  public void onEvent(
      final Event event, final int currentAutoCapsState, final int currentRecapitalizeState) {
  ...
  public View onCreateInputView(final int uiMode) {
    ...
    mCurrentInputView =
        (InputView) LayoutInflater.from(mThemeContext).inflate(R.layout.input_view, null);
