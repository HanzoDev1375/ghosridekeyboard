...
/**
 * Container for keys in the keyboard. All keys in a row are at the same Y-coordinate. Some of the
 * key size defaults can be overridden per row from what the {@link Keyboard} defines.
 */
public final class KeyboardRow {
  ...
  private static class RowAttributes {
    ...
    public RowAttributes(
        final TypedArray keyAttr, final float defaultKeyPaddedWidth, final float keyboardWidth) {
      mDefaultKeyPaddedWidth =
          ResourceUtils.getFraction(
              keyAttr, R.styleable.Keyboard_Key_keyWidth, keyboardWidth, defaultKeyPaddedWidth);
      mDefaultKeyLabelFlags = keyAttr.getInt(R.styleable.Keyboard_Key_keyLabelFlags, 0);
      mDefaultBackgroundType =
          keyAttr.getInt(R.styleable.Keyboard_Key_backgroundType, Key.BACKGROUND_TYPE_NORMAL);
    }

    /**
     * Parse and update key attributes using default attributes. This constructor is used to parse
     * include tag.
     *
     ...
     */
    public RowAttributes(
        final TypedArray keyAttr, final RowAttributes defaultRowAttr, final float keyboardWidth) {
      mDefaultKeyPaddedWidth =
          ResourceUtils.getFraction(
              keyAttr,
      ...
      mDefaultBackgroundType =
          keyAttr.getInt(
              R.styleable.Keyboard_Key_backgroundType, defaultRowAttr.mDefaultBackgroundType);
    }
  }

  public KeyboardRow(
      final Resources res, final KeyboardParams params, final XmlPullParser parser, final float y) {
    mParams = params;
    ...
    if (rowEndY > keyboardBottomEdge - FLOAT_THRESHOLD) {
      ...
      if (keyOverflow > FLOAT_THRESHOLD) {
        if (Math.round(keyOverflow) > 0) {
          ...
          Log.e(
              TAG,
              "The row is too tall to fit in the keyboard ("
                  + keyOverflow
    ...
    mRowAttributesStack.push(
        new RowAttributes(keyAttr, params.mDefaultKeyPaddedWidth, params.mBaseWidth));
    keyAttr.recycle();
  ...
  public void pushRowAttributes(final TypedArray keyAttr) {
    final RowAttributes newAttributes =
        new RowAttributes(keyAttr, mRowAttributesStack.peek(), mParams.mBaseWidth);
  ...
  public void updateXPos(final TypedArray keyAttr) {
    ...
    final float keyXPos =
        ResourceUtils.getFraction(keyAttr, R.styleable.Keyboard_Key_keyXPos, mParams.mBaseWidth, 0)
    ...
    if (keyXPos + FLOAT_THRESHOLD < mLastKeyRightEdge) {
      Log.e(
          TAG,
          "The specified keyXPos ("
              + keyXPos
              + ") is smaller than the next available x position ("
              + mLastKeyRightEdge
  ...
  /**
   * Determine the next key's dimensions so they can be retrieved using {@link #getKeyX()}, {@link
   * #getKeyWidth()}, etc.
   *
   * @param keyAttr the Key XML attributes array.
  ...
  public void setCurrentKey(final TypedArray keyAttr, final boolean isSpacer) {
    ...
    float keyWidth;
    if (isSpacer) {
      final float leftGap = Math.min(mNextKeyXPos - mLastKeyRightEdge - defaultGap, defaultGap);
    ...
    if (keyOverflow > FLOAT_THRESHOLD) {
      if (Math.round(keyOverflow) > 0) {
        ...
        Log.e(
            ...
            "The "
                + (isSpacer ? "spacer" : "key")
                + " is too wide to fit in the keyboard ("
                + keyOverflow
