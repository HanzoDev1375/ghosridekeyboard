...
public class RichInputMethodManager {
  ...
  /**
   * Manager for the list of enabled subtypes that also handles which one is currently in use. Only
   * one of these should be created to avoid conflicts.
   */
  private static class SubtypeList {
    /**
     * The list of enabled subtypes ordered by how they should be cycled through when moving to the
     * next subtype. When a subtype is actually in use, it should be moved to the beginning of the
     * list so that the next time the user uses the switch to next subtype button, all of the
     * subtypes can be iterated through before potentially switching to a different input method.
     */
    private final List<Subtype> mSubtypes;

    /**
     * The index of the currently selected subtype. This is used for tracking the status of cycling
     * through subtypes. When actually using the keyboard, the subtype should be moved to the
     * beginning of the list, so this should normally be 0.
     */
    ...
    public SubtypeList(final Context context) {
      ...
      final List<Subtype> subtypes =
          SubtypePreferenceUtils.createSubtypesFromPref(prefSubtypes, context.getResources());
    ...
    public synchronized Set<Subtype> getAllForLocale(final String locale) {
      ...
      for (final Subtype subtype : mSubtypes) {
        if (subtype.getLocale().equals(locale)) subtypes.add(subtype);
      }
      return subtypes;
    }
    ...
    /**
     * Move the current subtype to the beginning of the list to allow the rest of the subtypes to be
     * cycled through before possibly switching to a separate input method. This should be called
     * whenever the user is done cycling through subtypes (eg: when a subtype is actually used or
     ...
     *
     * @param notifyChangeOnCycle whether the subtype changed handler should be notified if the end
     *     of the list is passed and the next subtype would go back to the first in the list.
   ...
   * Get all of the enabled subtypes.
   *
   * @param sortForDisplay whether the subtypes should be sorted alphabetically by the display name
   *     as opposed to having no particular order.
   * @return the enabled subtypes.
   */
  ...
  /**
   * Move the current subtype to the beginning of the list to allow the rest of the subtypes to be
   * cycled through before possibly switching to a separate input method.
   */
   ...
   *
   * @param token supplies the identifying token given to an input method when it was started, which
   *     allows it to perform this operation on itself.
   ...
   */
  public AlertDialog showSubtypePicker(
    ...
    for (final SubtypeInfo subtypeInfo : subtypeInfoList) {
      ...
      itemTitle.setSpan(
          new RelativeSizeSpan(0.9f), 0, itemTitle.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
            ...
            for (final SubtypeInfo subtypeInfo : subtypeInfoList) {
              if (i == position) {
                if (subtypeInfo.virtualSubtype != null) {
                  ...
                  switchToTargetIme(
                      subtypeInfo.imiId, subtypeInfo.systemSubtype, inputMethodService);
                }
                break;
              }
              i++;
  ...
  private List<SubtypeInfo> getEnabledSubtypeInfoOfAllImes(final Context context) {
    ...
    final Set<InputMethodInfo> imiList =
        new TreeSet<>(
  ...
  private void switchToTargetIme(
      final String imiId, final InputMethodSubtype subtype, final InputMethodService context) {
