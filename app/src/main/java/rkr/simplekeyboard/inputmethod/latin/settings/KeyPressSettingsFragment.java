 ...
 *
 * <p>This settings sub screen handles the following input preferences. - Vibrate on keypress -
 * Keypress vibration duration - Sound on keypress - Keypress sound volume - Popup on keypress - Key
 * long press delay
 */
public final class KeyPressSettingsFragment extends SubScreenFragment {
  ...
  private void setupKeypressVibrationDurationSettings() {
    final SliderDialogPreference pref =
        (SliderDialogPreference) findPreference(Settings.PREF_VIBRATION_DURATION_SETTINGS);
    if (pref == null) {
      return;
    }
    ...
    pref.setInterface(
        new SliderDialogPreference.ValueProxy() {
          ...
          public String getValueText(final int value) {
            if (value < 0) {
              return getString(R.string.settings_system_default);
            }
            return getString(R.string.abbreviation_unit_milliseconds, value);
          }
        });
  }
  ...
  private void setupKeypressSoundVolumeSettings() {
    final SliderDialogPreference pref =
        (SliderDialogPreference) findPreference(Settings.PREF_KEYPRESS_SOUND_VOLUME);
    if (pref == null) {
      return;
    }
    final SharedPreferences prefs = getSharedPreferences();
  ...
  private void setupKeyLongpressTimeoutSettings() {
    ...
    final Resources res = getResources();
    final SliderDialogPreference pref =
        (SliderDialogPreference) findPreference(Settings.PREF_KEY_LONGPRESS_TIMEOUT);
    if (pref == null) {
      return;
    }
    pref.setInterface(
