...
public final class SingleLanguageSettingsFragment extends PreferenceFragmentCompat {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    if (args != null) {
      var locale = getArguments().getString(LOCALE_BUNDLE_KEY);
  ...
  private void buildContent(final String locale, final Context context) {
    ...
    group.setTitle("Language settings");
   ...
   * @param context the context for this application.
   */
  private void buildSubtypePreferences(
      final String locale, final PreferenceGroup group, final Context context) {
    final Set<Subtype> enabledSubtypes = mRichImm.getEnabledSubtypes(false);
    final List<Subtype> subtypes = SubtypeLocaleUtils.getSubtypes(locale, context.getResources());
   ...
   */
  private SubtypePreference createSubtypePreference(
      final Subtype subtype, final boolean checked, final Context context) {
  ...
  private static class SubtypePreference extends SwitchPreferenceCompat {
